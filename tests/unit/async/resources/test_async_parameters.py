...
from openaq.shared.responses import ParametersResponse, LatestResponse
...
class TestParameters:
    ...
    async def test_latest_calls_client_correctly(
        ...
        result = await parameters.latest(2)
    ...
    async def test_list_with_pagination(
        ...
        mock_client._get.return_value = mock_list_response
        await parameters.list(page=3, limit=50)
        params = mock_client._get.call_args[1]["params"]
    ...
    async def test_list_with_sorting(
        ...
        mock_client._get.return_value = mock_list_response
        await parameters.list(order_by="id", sort_order=sort_order)
        params = mock_client._get.call_args[1]["params"]
    ...
    async def test_parameters_get_throws(self, parameters, value):
        with pytest.raises(IdentifierOutOfBoundsError):
            await parameters.get(value)
    ...
    async def test_parameters_latest_throws(self, parameters, value):
        with pytest.raises(IdentifierOutOfBoundsError):
            await parameters.latest(value)
    ...
    async def test_parameters_list_throws(self, parameters, parameter, value):
        mock_params = {parameter: value}
        with pytest.raises(InvalidParameterError):
            await parameters.list(**mock_params)
