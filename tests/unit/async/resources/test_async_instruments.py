...
class TestInstruments:
    async def test_get_calls_client_correctly(
        ...
        result = await instruments.get(12)
    ...
    async def test_list_with_pagination(
        ...
        mock_client._get.return_value = mock_list_response
        await instruments.list(page=3, limit=50)
        params = mock_client._get.call_args[1]["params"]
    ...
    async def test_list_with_sorting(
        ...
        mock_client._get.return_value = mock_list_response
        await instruments.list(order_by="id", sort_order=sort_order)
        params = mock_client._get.call_args[1]["params"]
    ...
    async def test_instruments_get_throws(self, instruments, value):
        with pytest.raises(IdentifierOutOfBoundsError):
            await instruments.get(value)
    ...
    async def test_instruments_list_throws(self, instruments, parameter, value):
        mock_params = {parameter: value}
        with pytest.raises(InvalidParameterError):
            await instruments.list(**mock_params)
