...
class TestOwners:
    ...
    async def test_list_with_pagination(self, owners, mock_client, mock_list_response):
        mock_client._get.return_value = mock_list_response
        await owners.list(page=3, limit=50)
        params = mock_client._get.call_args[1]["params"]
    ...
    async def test_list_with_sorting(
        ...
        mock_client._get.return_value = mock_list_response
        await owners.list(order_by="id", sort_order=sort_order)
        params = mock_client._get.call_args[1]["params"]
    ...
    async def test_owners_get_throws(self, owners, value):
        with pytest.raises(IdentifierOutOfBoundsError):
            await owners.get(value)
    ...
    async def test_owners_list_throws(self, owners, parameter, value):
        mock_params = {parameter: value}
        with pytest.raises(InvalidParameterError):
            await owners.list(**mock_params)
